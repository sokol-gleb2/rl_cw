    ...
    **ONLY CHANGE THE BODY OF THE act() FUNCTION**
    ...
    def act(self, obs: int) -> int:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        """
        prob = random.random()
        # Check if any actions exist for obs
        actions_for_obs = [a for (o, a) in self.q_table if o == obs]

        if prob < self.epsilon or not actions_for_obs:
            # Unif(A)
            a = self.action_space.sample()
        else:
            # Exploitation → pick best known action
            a = max(actions_for_obs, key=lambda a: self.q_table.get((obs, a), 0))
        return a
...
class QLearningAgent(Agent):
    ...
    def learn(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        # Get max Q-value for the next state (if not terminal)
        if done:
            max_future_q = 0
        else:
            # Find max Q-value for any action in `n_obs`
            max_future_q = max(
                (self.q_table.get((n_obs, a), 0) for a in range(self.action_space.n)), 
                default=0
            )

        self.q_table[(obs, action)] = self.q_table[(obs, action)] + self.alpha*(reward + self.gamma * max_future_q - self.q_table[(obs, action)])
...
class MonteCarloAgent(Agent):
    ...
    def learn(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        # Reverse iterate over episode to calculate returns
        for t in reversed(range(len(obses))):
            obs, action, reward = obses[t], actions[t], rewards[t]
            G = reward + self.gamma * G  # Compute return G_t

            # Check if (obs, action) has appeared before in the episode (Every-Visit MC)
            if (obs, action) not in obses[:t]:  # Ensures Every-Visit property
                # Count occurrences of (obs, action)
                ...
                updated_q_value = old_q_value + (1 / self.sa_counts[(obs, action)]) * (G - old_q_value)

                # Store updated value
                self.q_table[(obs, action)] = updated_q_value
                updated_values[(obs, action)] = updated_q_value
