...
class ValueIteration(MDPSolver):
        ...
        **DO NOT ALTER THE MDP HERE**
        ...
        while True:
            ...
            for s_idx in range(self.state_dim):
                v = V[s_idx]
                V[s_idx] = np.max([
                    np.sum(self.mdp.P[s_idx, a_idx, :] * (self.mdp.R[s_idx, a_idx, :] + self.gamma * V))
    ...
    def _calc_policy(self, V: np.ndarray) -> np.ndarray:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        for (s_idx, s) in enumerate(self.mdp.states):
            best_action = np.argmax([
                np.sum(self.mdp.P[s_idx, a_idx, :] * (self.mdp.R[s_idx, a_idx, :] + self.gamma * V))
                for a_idx in range(self.action_dim)
            ])
            policy[s_idx, best_action] = 1.0  # Assign deterministic policy
        return policy
...
class PolicyIteration(MDPSolver):
    ...
    def _policy_eval(self, policy: np.ndarray) -> np.ndarray:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while True:
            ...
            for s_idx in range(self.state_dim):
                v = V[s_idx]
                a_idx = np.argmax(policy[s_idx])  # Select deterministic action
                V[s_idx] = np.sum(
                    self.mdp.P[s_idx, a_idx, :] * (self.mdp.R[s_idx, a_idx, :] + self.gamma * V)
                )
                delta = max(delta, abs(v - V[s_idx]))
    ...
    def _policy_improvement(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while not stable:
            V = self._policy_eval(policy)  # Compute value function of current policy
            ...
            for s_idx in range(self.state_dim):
                ...
                new_policy[s_idx, best_action] = 1.0  # Assign deterministic policy
                if old_action != best_action:
                    stable = False  # Policy changed, so it's not stable yet
